...
export default function ARUpload() {
  ...
  useEffect(() => {
    ...
    if (!address) {
      try {
        localStorage.setItem("arweave-key", JSON.stringify(previousKey));
        setJwk(previousKey);
        getKeyForJwk(previousKey).then((a) => {
          setAddress(a);
        });
      } catch (e) {
        console.log(e);
    ...
    <>
      ...
      <div>
        <p className="px-2 text-center">
          This tool lets you upload files to arweave. Currently limited to 150mb total per batch of files. <br />
            ...
            <div className="card">
              <div className="card-body">
                <Form form={jwkForm}>
                  ...
                  <Form.Item>
                    <button
                      className={`btn btn-primary rounded inline-block mx-auto ${
                        loading ? "loading" : ""
                      }`}
                      onClick={() => generate()}
                    >
